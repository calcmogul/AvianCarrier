...
std::string Patch::toString() {
  ...
  if (length1 == 0) {
    coords1 = intString(start1) + std::string(",0");
  } else if (length1 == 1) {
    coords1 = intString(start1 + 1);
  } else {
    coords1 = intString(start1 + 1) + std::string(",")
        + intString(length1);
  }
  if (length2 == 0) {
    coords2 = intString(start2) + std::string(",0");
  } else if (length2 == 1) {
    coords2 = intString(start2 + 1);
  } else {
    coords2 = intString(start2 + 1) + std::string(",")
        + intString(length2);
  }
  std::string text;
  ...
  for_each (Diff aDiff, diffs) {
    ...
    text += std::string(QUrl::toPercentEncoding(aDiff.text, " !~*'();/?:@&=+$,#"))
        + std::string("\n");
  }
...
std::list<Diff> diff_match_patch::diff_main(const std::string &text1,
...
std::list<Diff> diff_match_patch::diff_main(const std::string &text1,
  ...
  const std::string &commonprefix = text1.left(commonlength);
  std::string textChopped1 = text1.mid(commonlength);
  std::string textChopped2 = text2.mid(commonlength);

  // Trim off common suffix (speedup).
  commonlength = diff_commonSuffix(textChopped1, textChopped2);
  const std::string &commonsuffix = textChopped1.right(commonlength);
...
std::list<Diff> diff_match_patch::diff_compute(std::string text1, std::string text2,
  ...
  {
    const std::string longtext = text1.length() > text2.length() ? text1 : text2;
    const std::string shorttext = text1.length() > text2.length() ? text2 : text1;
  ...
  const std::list<std::string> hm = diff_halfMatch(text1, text2);
  if (hm.count() > 0) {
...
std::list<Diff> diff_match_patch::diff_lineMode(std::string text1, std::string text2,
    clock_t deadline) {
  // Scan the text on a line-by-line basis first.
  const std::list<QVariant> b = diff_linesToChars(text1, text2);
  text1 = b[0].toString();
  text2 = b[1].toString();
  std::list<std::string> linearray = b[2].toStringList();
...
std::list<Diff> diff_match_patch::diff_bisect(const std::string &text1,
...
std::list<Diff> diff_match_patch::diff_bisectSplit(const QString &text1,
    const QString &text2, int x, int y, clock_t deadline) {
  std::string text1a = text1.left(x);
  std::string text2a = text2.left(y);
...
std::list<QVariant> diff_match_patch::diff_linesToChars(const std::string &text1,
                                                    const std::string &text2) {
  std::list<std::string> lineArray;
  ...
  const std::string chars1 = diff_linesToCharsMunge(text1, lineArray, lineHash);
  const std::string chars2 = diff_linesToCharsMunge(text2, lineArray, lineHash);

  std::list<QVariant> listRet;
...
std::string diff_match_patch::diff_linesToCharsMunge(const std::string &text,
                                                 std::list<std::string> &lineArray,
                                                 std::map<std::string, int> &lineHash) {
  int lineStart = 0;
  int lineEnd = -1;
  ...
  while (lineEnd < text.length() - 1) {
    ...
    if (lineHash.contains(line)) {
      chars += static_cast<unsigned char>((static_cast<ushort>(lineHash.value(line)));
    } else {
      lineArray.append(line);
      lineHash.insert(line, lineArray.size() - 1);
      chars += static_cast<unsigned char>(static_cast<ushort>(lineArray.size() - 1));
    }
  }
  return chars;
}
...
void diff_match_patch::diff_charsToLines(std::list<Diff> &diffs,
                                         const std::list<std::string> &lineArray) {
...
int diff_match_patch::diff_commonOverlap(const std::string &text1,
  ...
  int length = 1;
  while (true) {
	std::string pattern = text1_trunc.right(length);
...
std::list<std::string> diff_match_patch::diff_halfMatch(const std::string &text1,
                                             const std::string &text2) {
  if (Diff_Timeout <= 0) {
  ...
  if (longtext.length() < 4 || shorttext.length() * 2 < longtext.length()) {
    return std::list<std::string>();  // Pointless.
  }

  // First check if the second quarter is the seed for a half-match.
  const std::list<std::string> hm1 = diff_halfMatchI(longtext, shorttext,
      (longtext.length() + 3) / 4);
  // Check again based on the third quarter.
  const std::list<std::string> hm2 = diff_halfMatchI(longtext, shorttext,
      (longtext.length() + 1) / 2);
  std::list<std::string> hm;
...
std::list<std::string> diff_match_patch::diff_halfMatchI(const std::string &longtext,
                                              const std::string &shorttext,
                                              int i) {
  // Start with a 1/4 length substring at position i as a seed.
  const std::string seed = safeMid(longtext, i, longtext.length() / 4);
  int j = -1;
...
void diff_match_patch::diff_cleanupSemantic(std::list<Diff> &diffs) {
  ...
  std::string lastequality;  // Always equal to equalities.lastElement().text
  QMutableListIterator<Diff> pointer(diffs);
  ...
  while (thisDiff != NULL) {
    if (prevDiff->operation == DELETE &&
      ...
      std::string deletion = prevDiff->text;
      std::string insertion = thisDiff->text;
...
void diff_match_patch::diff_cleanupSemanticLossless(std::list<Diff> &diffs) {
  std::string equality1, edit, equality2;
  std::string commonString;
  int commonOffset;
  int score, bestScore;
  std::string bestEquality1, bestEdit, bestEquality2;
...
int diff_match_patch::diff_cleanupSemanticScore(const std::string &one,
  ...
  unsigned char char1 = one[one.length() - 1];
  unsigned char char2 = two[0];
...
std::string diff_match_patch::diff_prettyHtml(const QList<Diff> &diffs) {
  std::string html;
  std::string text;
  foreach(Diff aDiff, diffs) {
    text = aDiff.text;
...
std::string diff_match_patch::diff_toDelta(const std::list<Diff> &diffs) {
  ...
  foreach(Diff aDiff, diffs) {
    switch (aDiff.operation) {
      case INSERT: {
    	std::string encoded = std::string(QUrl::toPercentEncoding(aDiff.text,
            " !~*'();/?:@&=+$,#"));
        text += std::string("+") + encoded + std::string("\t");
        break;
      }
      case DELETE:
        text += std::string("-") + std::string::number(aDiff.text.length())
            + std::string("\t");
        break;
      case EQUAL:
        text += std::string("=") + std::string::number(aDiff.text.length())
...
std::list<Diff> diff_match_patch::diff_fromDelta(const std::string &text1,
  ...
  std::list<std::string> tokens = delta.split("\t");
  foreach(std::string token, tokens) {
    ...
    std::string param = safeMid(token, 1);
    switch (token[0].toAscii()) {
      ...
      default:
        throw std::string("Invalid diff operation in diff_fromDelta: %1")
            .arg(token[0]);
    }
  }
...
int diff_match_patch::match_bitap(const std::string &text, const std::string &pattern,
  ...
  std::map<unsigned char, int> s = match_alphabet(pattern);
...
std::map<unsigned char, int> diff_match_patch::match_alphabet(const QString &pattern) {
  std::map<unsigned char, int> s;
  int i;
...
std::pair<std::string, std::vector<bool> > diff_match_patch::patch_apply(
    std::list<Patch> &patches, const QString &sourceText) {
  std::string text = sourceText;  // Copy to preserve original.
  if (patches.isEmpty()) {
    return std::pair<std::string,std::vector<bool> >(text, std::vector<bool>(0));
  }
...
std::string diff_match_patch::patch_addPadding(QList<Patch> &patches) {
  short paddingLength = Patch_Margin;
  std::string nullPadding = "";
  for (short x = 1; x <= paddingLength; x++) {
    nullPadding += static_cast<unsigned char>((ushort)x);
  }
...
std::list<Patch> diff_match_patch::patch_fromText(const std::string &textline) {
  ...
  std::list<std::string> text = textline.split("\n", std::string::SkipEmptyParts);
  Patch patch;
  ...
  while (!text.isEmpty()) {
    ...
    while (!text.isEmpty()) {
      ...
      if (sign == '-') {
        ...
        // WTF?
        throw std::string("Invalid patch mode '%1' in: %2").arg(sign).arg(line);
